...
public class NuevaActividad extends AppCompatActivity implements AdapterView.OnItemSelectedListener {
    ...
    public void guardar(View v){
        ...
        Actividad a = new Actividad("",idprof,tipo,lugars,lugarl,fechaS,fechaL,"pablo",descripcion);

        final Call<Actividad> callActi = api.createActividad(a);
        callActi.enqueue(new Callback<Actividad>() {
            @Override
            public void onResponse(Response<Actividad> response, Retrofit retrofit) {
                Log.v("XXXXXXX: "+idprof, "Actividad Subida: "+retrofit.baseUrl());
                setResult(RESULT_OK);
                finish();
            }
